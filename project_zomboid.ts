...
import { LodestoneTypes, LodestoneInstanceState } from "./utils/types.ts";
...
export default class ProjectZomboidInstance extends ImplInstance {
    ...
    public async setup(
        ...
        let port_1: number = this.identity(setupValue, "game_conf", "port_1", LodestoneTypes.UnsignedInteger);
        let port_2: number = this.identity(setupValue, "game_conf", "port_2", LodestoneTypes.UnsignedInteger);
        ...
        await this.saveConfig(this.config, `${path}/${ProjectZomboidInstance.restoreConfigName}`)
    ...
    public async restore(
        ...
        this.uuid = dotLodestoneConfig.uuid;
        this.config = await this.readConfig(`${path}/${ProjectZomboidInstance.restoreConfigName}`) as ProjectZomboidConfig;
        this.event_stream = new EventStream(this.uuid, this.config.name);
    ...
    public async start(caused_by: Atom.CausedBy, block: boolean): Promise<void> {
        ...
        const cmd = new Deno.Command(`${this.config.path}/start-server.sh`, { stdin: "piped", stderr: "piped", stdout: "piped" }).spawn()
        this._console_handlers._consoleWriter = cmd.stdin.getWriter()
        this._console_handlers._consoleReader = mergeReadableStreams(cmd.stdout, cmd.stderr).getReader()
    ...
    private async _Install(steamAppId: string, path: string) {
        const tmpcmd = new Deno.Command("which", { args: ["steamcmd"], stdout: "null", stderr: "null" }).spawn()
    ...
    private writeConsole(text: string) {
        this.checkConsole()
        this._console_handlers._consoleWriter?.write(this._console_handlers._textEncoder.encode(text))
    }
